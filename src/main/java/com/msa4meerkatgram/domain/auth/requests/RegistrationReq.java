...
public record RegistrationReq(
    ...
    @Schema(description = "이메일 필수", example = "test@test.com", nullable = false, requiredMode = Schema.RequiredMode.REQUIRED)
    @NotBlank(message = "이메일은 필수 항목 입니다.")
    @Pattern(regexp = "^[0-9a-zA-Z](?!.*?[\\-_.]{2})[a-zA-Z0-9\\-_.]{3,63}@[0-9a-zA-Z](?!.*?[\\-_.]{2})[a-zA-Z0-9\\-_.]{3,63}\\.[a-zA-Z]{2,3}$", message = "허용하지 않는 이메일 양식입니다.")
    String email,

    @Schema(description = "비밀번호 필수", example = "qwer1234", nullable = false, requiredMode = Schema.RequiredMode.REQUIRED)
    ...
    @Schema(description = "닉네임", example = "test", nullable = false, requiredMode = Schema.RequiredMode.REQUIRED)
    @NotBlank(message = "닉네임은 필수 항목 입니다.")
    @Pattern(regexp = "^[0-9a-zA-Z_]{2,20}$", message = "허용하지 않는 닉네임 양식입니다.")
    String nick,

    @Schema(description = "프로필", example = "http://localhost:8080/files/posts/20260611_2b8633ed-d421-4c27-bd23-f7f0ed448067.jpg", nullable = false, requiredMode = Schema.RequiredMode.REQUIRED)
